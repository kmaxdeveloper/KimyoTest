...
class FindFileFromDevice {
    fun findPdfFile(fileName: String): String {
        val directories = listOf(
            ...
            File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "KimyoTest/Kitoblar")
        )
    ...
    fun Context.findFileInSecureDir(folderName: String, fileName: String): File? {
        val secureDir = File(filesDir, folderName) // 📂 /data/data/{package_name}/files/KimyoTest/
    ...
    // 📌 API 24-28 uchun `Downloads/KimyoTest/Kitoblar` da qidirish
