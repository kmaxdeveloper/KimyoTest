package uz.kmax.kimyotest.data.tools.firebase

import com.google.firebase.database.*

class FirebaseManager() {

    private val database = FirebaseDatabase.getInstance().getReference("KimyoTest")
    ...
    // Ma'lumotni bir marta ro'yxat ko'rinishida o'qish //
    fun <T> readList(path: String, clazz: Class<T>, onDataChange: (ArrayList<T>?) -> Unit) {
        database.child(path).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
