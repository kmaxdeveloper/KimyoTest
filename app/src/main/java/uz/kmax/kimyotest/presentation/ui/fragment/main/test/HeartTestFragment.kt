...
import uz.kmax.kimyotest.databinding.FragmentTestHeartBinding
...
import uz.kmax.kimyotest.presentation.ui.dialog.DialogOverHeart
import uz.kmax.kimyotest.presentation.ui.fragment.main.MenuFragment
import javax.inject.Inject
import kotlin.random.Random

@AndroidEntryPoint
class HeartTestFragment : BaseFragmentWC<FragmentTestHeartBinding>(FragmentTestHeartBinding::inflate) {
    ...
    companion object {
        ...
        fun newInstance(testLocation: String, testCount: Int): HeartTestFragment {
            val fragment = HeartTestFragment()
            val args = Bundle()
        ...
        if (testManager.checkAnswerBoolean(variantList[position].text.toString())) {
            ...
            if (heart != 1) {
                ...
                if (adsManager.admobRewardedAdsReady()) {
                    dialogOverHeart.show(ctx)
                    dialogOverHeart.setOnViewAdListener {
                        val currentActivity = activity ?: return@setOnViewAdListener
                        adsManager.showRewardedAds(currentActivity) { reward, adsStatus ->
                            heart = 0
                            heart += reward
