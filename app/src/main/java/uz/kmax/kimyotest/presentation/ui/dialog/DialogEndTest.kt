package uz.kmax.kimyotest.presentation.ui.dialog
...
import uz.kmax.kimyotest.databinding.DialogEndTestBinding
...
class DialogEndTest {
    ...
    fun show(context: Context, correctAnswerCount: Int, wrongAnswerCount: Int, testType : Int = 1) {
        ...
        var remainAnswer = 30 - (wrongAnswerCount + correctAnswerCount)
        binding.remainAnswerCount.text = remainAnswer.toString()
        when(testType){
            1->{
                binding.remainAnswerLayout.visibility = View.GONE
            }

            3->{
                binding.remainAnswerLayout.visibility = View.VISIBLE
            }
